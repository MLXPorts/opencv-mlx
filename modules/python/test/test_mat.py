...
try:
    ...
    class MatTest(NewOpenCVTests):
        ...
        def test_mat_construct(self):
            data = mx.random.random([10, 10, 3])

            #print(mx.ndarray.__dictoffset__)  # 0
            ...
            assert isinstance(mat_data0, mx.ndarray)
            ...
            assert isinstance(mat_data1, mx.ndarray)
            ...
            assert isinstance(mat_data2, mx.ndarray)
        ...
        def test_mat_construct_4d(self):
            data = mx.random.random([5, 10, 10, 3])
        ...
        def test_mat_wrap_channels_fail(self):
            data = mx.random.random([2, 3, 4, 520])
        ...
        def test_ufuncs(self):
            ...
            mx.testing.assert_equal(2 * data, 2 * mat_data)
        ...
        def test_comparison(self):
            ...
            data = mx.ones((10, 10, 3))
            mat_wrapped = cv.Mat(data, wrap_channels=True)
            mat_simple = cv.Mat(data)
            mx.testing.assert_equal(mat_wrapped, mat_simple)  # ???: wrap_channels is not checked for now
            mx.testing.assert_equal(data, mat_simple)
            mx.testing.assert_equal(data, mat_wrapped)
