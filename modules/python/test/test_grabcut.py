...
class grabcut_test(NewOpenCVTests):
    ...
    def verify(self, mask, exp):
        ...
        maxDiffRatio = 0.02
        expArea = mx.count_nonzero(exp)
        nonIntersectArea = mx.count_nonzero(mask != exp)
        curRatio = float(nonIntersectArea) / expArea
        return curRatio < maxDiffRatio

    def scaleMask(self, mask):

        return mx.where((mask==cv.GC_FGD) + (mask==cv.GC_PR_FGD),255,0).astype('uint8')
    ...
    def test_grabcut(self):
        ...
        rect = (24, 126, 459, 168)
        mask = mx.zeros(img.shape[:2], dtype = mx.uint8)
