...
def blur_edge(img, d=31):
    ...
    y, x = mx.indices((h, w))
    dist = mx.dstack([x, w-x-1, y, h-y-1]).min(-1)
    w = mx.minimum(mx.float32(dist)/d, 1.0)
    return img*w + img_blur*(1-w)

def motion_kernel(angle, d, sz=65):
    kern = mx.ones((1, d), mx.float32)
    c, s = mx.cos(angle), mx.sin(angle)
    A = mx.float32([[c, -s, 0], [s, c, 0]])
    sz2 = sz // 2
    A[:,2] = (sz2, sz2) - mx.dot(A[:,:2], ((d-1)*0.5, 0))
    kern = cv.warpAffine(kern, A, (sz, sz), flags=cv.INTER_CUBIC)
    return kern

def defocus_kernel(d, sz=65):
    kern = mx.zeros((sz, sz), mx.uint8)
    cv.circle(kern, (sz, sz), d, 255, -1, cv.LINE_AA, shift=1)
    kern = mx.float32(kern) / 255.0
    return kern
...
def main():
    ...
    def update(_):
        ang = mx.deg2rad( cv.getTrackbarPos('angle', win) )
