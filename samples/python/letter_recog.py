...
def main():
    ...
    print('testing...')
    train_rate = mx.mean(model.predict(samples[:train_n]) == responses[:train_n].astype(int))
    test_rate  = mx.mean(model.predict(samples[train_n:]) == responses[train_n:].astype(int))
