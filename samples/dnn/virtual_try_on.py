...
def get_pose_map(image, proto_path, model_path, backend, target, height=256, width=192):
    ...
    pose_map = mx.zeros((height, width, out_c - 1))
    # last label: Background
    for i in range(0, out.shape[1] - 1):
        heatMap = out[0, i, :, :]
        keypoint = mx.full((height, width), -1)
...
class BilinearFilter(object):
    ...
    def _precompute_coeffs(self, inSize, outSize):
        ...
        ksize = int(mx.ceil(filterscale)) * 2 + 1

        kk = mx.zeros(shape=(outSize * ksize, ), dtype=mx.float32)
        bounds = mx.empty(shape=(outSize * 2, ), dtype=mx.int32)

        centers = (mx.arange(outSize) + 0.5) * filterscale + 0.5
        bounds[::2] = mx.where(centers - filterscale < 0, 0, centers - filterscale)
        bounds[1::2] = mx.where(centers + filterscale > inSize, inSize, centers + filterscale) - bounds[::2]
        xmins = bounds[::2] - centers + 1

        points = mx.array([mx.arange(row) + xmins[i] for i, row in enumerate(bounds[1::2])]) / filterscale
        for xx in range(0, outSize):
            point = points[xx]
            bilinear = mx.where(point < 1.0, 1.0 - abs(point), 0.0)
            ww = mx.sum(bilinear)
            kk[xx * ksize : xx * ksize + bilinear.size] = mx.where(ww == 0.0, bilinear, bilinear / ww)
    ...
    def _resample_horizontal(self, out, img, ksize, bounds, kk):
        for yy in range(0, out.shape[0]):
            for xx in range(0, out.shape[1]):
                ...
                out[yy, xx] = mx.round(mx.sum(img[yy, xmin : xmin + xmax] * k))
    ...
    def _resample_vertical(self, out, img, ksize, bounds, kk):
        for yy in range(0, out.shape[0]):
            ...
            out[yy] = mx.round(mx.sum(img[ymin : ymin + ymax, 0:out.shape[1]] * k[:, mx.newaxis], axis=0))
    ...
    def imaging_resample(self, img, xsize, ysize):
        ...
        out_hor = mx.empty((img.shape[0], xsize), dtype=mx.uint8)
        self._resample_horizontal(out_hor, img, ksize_horiz, bounds_horiz, kk_horiz)
        out = mx.empty((ysize, xsize), dtype=mx.uint8)
...
class CpVton(object):
    ...
    def prepare_agnostic(self, segm_image, input_image, pose_map, height=256, width=192):
        ...
        phead = mx.zeros((1, height, width), dtype=mx.float32)
        pose_shape = mx.zeros((height, width), dtype=mx.uint8)
        ...
        agnostic = mx.concatenate((res_shape, img_head, pose_map), axis=0)
        agnostic = mx.expand_dims(agnostic, axis=0)
        return agnostic.astype(mx.float32)
    ...
    def get_warped_cloth(self, cloth_img, agnostic, height=256, width=192):
        ...
        warped_cloth = self._bilinear_sampler(cloth, grid).astype(mx.float32)
        return warped_cloth

    def get_tryon(self, agnostic, warp_cloth):
        inp = mx.concatenate([agnostic, warp_cloth], axis=1)
        self.tom_net.setInput(inp)
        out = self.tom_net.forward()

        p_rendered, m_composite = mx.split(out, [3], axis=1)
        p_rendered = mx.tanh(p_rendered)
        m_composite = 1 / (1 + mx.exp(-m_composite))
    ...
    def _compute_L_inverse(self, X, Y):
        ...
        Xmat = mx.tile(X, (1, N))
        Ymat = mx.tile(Y, (1, N))
        P_dist_squared = mx.power(Xmat - Xmat.transpose(1, 0), 2) + mx.power(Ymat - Ymat.transpose(1, 0), 2)

        P_dist_squared[P_dist_squared == 0] = 1
        K = mx.multiply(P_dist_squared, mx.log(P_dist_squared))

        O = mx.ones([N, 1], dtype=mx.float32)
        Z = mx.zeros([3, 3], dtype=mx.float32)
        P = mx.concatenate([O, X, Y], axis=1)
        first = mx.concatenate((K, P), axis=1)
        second = mx.concatenate((P.transpose(1, 0), Z), axis=1)
        L = mx.concatenate((first, second), axis=0)
        Li = linalg.inv(L)
        return Li

    def _prepare_to_transform(self, out_h=256, out_w=192, grid_size=5):
        grid_X, grid_Y = mx.meshgrid(mx.linspace(-1, 1, out_w), mx.linspace(-1, 1, out_h))
        grid_X = mx.expand_dims(mx.expand_dims(grid_X, axis=0), axis=3)
        grid_Y = mx.expand_dims(mx.expand_dims(grid_Y, axis=0), axis=3)

        axis_coords = mx.linspace(-1, 1, grid_size)
        N = grid_size ** 2
        P_Y, P_X = mx.meshgrid(axis_coords, axis_coords)

        P_X = mx.reshape(P_X,(-1, 1))
        P_Y = mx.reshape(P_Y,(-1, 1))

        P_X = mx.expand_dims(mx.expand_dims(mx.expand_dims(P_X, axis=2), axis=3), axis=4).transpose(4, 1, 2, 3, 0)
        P_Y = mx.expand_dims(mx.expand_dims(mx.expand_dims(P_Y, axis=2), axis=3), axis=4).transpose(4, 1, 2, 3, 0)
    ...
    def _expand_torch(self, X, shape):
        if len(X.shape) != len(shape):
            ...
            return mx.tile(X, axis)

    def _apply_transformation(self, theta, points, N, P_X, P_Y):
        if len(theta.shape) == 2:
            theta = mx.expand_dims(mx.expand_dims(theta, axis=2), axis=3)

        batch_size = theta.shape[0]

        P_X_base = mx.copy(P_X)
        P_Y_base = mx.copy(P_Y)

        Li = self._compute_L_inverse(mx.reshape(P_X, (N, -1)), mx.reshape(P_Y, (N, -1)))
        Li = mx.expand_dims(Li, axis=0)

        # split theta into point coordinates
        Q_X = mx.squeeze(theta[:, :N, :, :], axis=3)
        Q_Y = mx.squeeze(theta[:, N:, :, :], axis=3)
        ...
        W_X = mx.expand_dims(mx.expand_dims(W_X, axis=3), axis=4).transpose(0, 4, 2, 3, 1)
        W_X = mx.repeat(W_X, points_h, axis=1)
        W_X = mx.repeat(W_X, points_w, axis=2)

        W_Y = mx.expand_dims(mx.expand_dims(W_Y, axis=3), axis=4).transpose(0, 4, 2, 3, 1)
        W_Y = mx.repeat(W_Y, points_h, axis=1)
        W_Y = mx.repeat(W_Y, points_w, axis=2)

        A_X = self._expand_torch(Li[:, N:, :N], (batch_size, 3, N)) @ Q_X
        A_Y = self._expand_torch(Li[:, N:, :N], (batch_size, 3, N)) @ Q_Y

        A_X = mx.expand_dims(mx.expand_dims(A_X, axis=3), axis=4).transpose(0, 4, 2, 3, 1)
        A_X = mx.repeat(A_X, points_h, axis=1)
        A_X = mx.repeat(A_X, points_w, axis=2)

        A_Y = mx.expand_dims(mx.expand_dims(A_Y, axis=3), axis=4).transpose(0, 4, 2, 3, 1)
        A_Y = mx.repeat(A_Y, points_h, axis=1)
        A_Y = mx.repeat(A_Y, points_w, axis=2)

        points_X_for_summation = mx.expand_dims(mx.expand_dims(points[:, :, :, 0], axis=3), axis=4)
        points_X_for_summation = self._expand_torch(points_X_for_summation, points[:, :, :, 0].shape + (1, N))

        points_Y_for_summation = mx.expand_dims(mx.expand_dims(points[:, :, :, 1], axis=3), axis=4)
        ...
        dist_squared = mx.power(delta_X, 2) + mx.power(delta_Y, 2)
        dist_squared[dist_squared == 0] = 1
        U = mx.multiply(dist_squared, mx.log(dist_squared))

        points_X_batch = mx.expand_dims(points[:,:,:,0], axis=3)
        points_Y_batch = mx.expand_dims(points[:,:,:,1], axis=3)
        ...
        points_X_prime = A_X[:,:,:,:,0]+ \
                        mx.multiply(A_X[:,:,:,:,1], points_X_batch) + \
                        mx.multiply(A_X[:,:,:,:,2], points_Y_batch) + \
                        mx.sum(mx.multiply(W_X, self._expand_torch(U, W_X.shape)), 4)

        points_Y_prime = A_Y[:,:,:,:,0]+ \
                        mx.multiply(A_Y[:,:,:,:,1], points_X_batch) + \
                        mx.multiply(A_Y[:,:,:,:,2], points_Y_batch) + \
                        mx.sum(mx.multiply(W_Y, self._expand_torch(U, W_Y.shape)), 4)

        return mx.concatenate((points_X_prime, points_Y_prime), 3)

    def _generate_grid(self, theta):
        grid_X, grid_Y, N, P_X, P_Y = self._prepare_to_transform()
        warped_grid = self._apply_transformation(theta, mx.concatenate((grid_X, grid_Y), axis=3), N, P_X, P_Y)
        return warped_grid
    ...
    def _bilinear_sampler(self, img, grid):
        ...
        x0 = mx.floor(x).astype(int)
        x1 = x0 + 1
        y0 = mx.floor(y).astype(int)
        y1 = y0 + 1
        ...
        x0 = mx.clip(x0, 0, max_x)
        x1 = mx.clip(x1, 0, max_x)
        y0 = mx.clip(y0, 0, max_y)
        y1 = mx.clip(y1, 0, max_y)
        ...
        wa = mx.expand_dims(wa, axis=0)
        wb = mx.expand_dims(wb, axis=0)
        wc = mx.expand_dims(wc, axis=0)
        wd = mx.expand_dims(wd, axis=0)
...
class CorrelationLayer(object):
    ...
    def forward(self, inputs):
        ...
        feature_A = mx.reshape(feature_A, (b, c, h * w))
        feature_B = mx.reshape(feature_B, (b, c, h * w))
        feature_B = feature_B.transpose(0, 2, 1)
        feature_mul = feature_B @ feature_A
        feature_mul= mx.reshape(feature_mul, (b, h, w, h * w))
        feature_mul = feature_mul.transpose(0, 1, 3, 2)
        correlation_tensor = feature_mul.transpose(0, 2, 1, 3)
        correlation_tensor = mx.ascontiguousarray(correlation_tensor)
